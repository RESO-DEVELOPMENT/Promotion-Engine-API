    ...
    public class ProductService : BaseService<Product, ProductDto>, IProductService
    {
        ...
        public async Task<bool> CheckExistin(string code, Guid brandId, Guid productId)
        {
            try
            {
                ...
                {
                    isExist = (await _repository.Get(filter: o => o.ProductCode.Equals(code)
        ...
        public async Task addProductFromSync(Guid brandId, ProductSyncParamDTO productSyncs)
        {
            ...
            foreach (var productSync in productSyncs.data.Products)
            {
                ...
                {
                    if (!listProduct.Any(a => a.ProductCode.Equals(productSync.Code)))
                    {
                    ...
                    {
                        var product = listProduct.Where(w => w.ProductCode.Equals(productSync.Code)).First();
                        product.ProductName = productSync.ProductName;
                ...
                else
                {
                    ...
                    {
                        if (!listProduct.Any(a => a.ProductCode.Equals(child.Code)))
                        {
                        ...
                        {
                            var product = listProduct.Where(w => w.ProductCode.Equals(child.Code)).First();
                            product.ProductName = productSync.ProductName;
                            _repository.Update(product);
                        }
                    }
