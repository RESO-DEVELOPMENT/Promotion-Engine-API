    ...
    public class ApplyPromotion : IApplyPromotion
    {
        ...
        private void DiscountProduct(Order order, Infrastructure.Models.Action action, Promotion promotion, PromotionTier promotionTier)
        {
            ...
            if (action.ActionType != (int)AppConstant.EnvVar.ActionType.Bundle)
            {
                ...
                {
                    if (actionProducts.Any(a => a.Product.ProductCode.Equals(product.ProductCode)))
                    {
                        var actionProduct = action.ActionProductMapping.FirstOrDefault(el => el.Product.ProductCode == product.ProductCode);
            ...
            else
            {
                ...
                foreach (var product in cartItems)
                {
                    bool isMatchProduct =
                            ...
                            .Any(el => el.Product.ProductCode == product.ProductCode
